...
// 🌟 FUNÇÃO DE AUTO-INSTALL DE COMPILADORES
void checarEInstalarCompilador() {
#ifdef _WIN32
    // 1. Verifica se o GCC está disponível no Windows
    ...
    if (!temGcc) {
        ...
        // Usa o winget para instalar o MSYS2/MinGW silenciosamente
        int res = system("winget install GNU.GCC --silent --accept-package-agreements --accept-source-agreements");
        
        if (res == 0) {
            printf("✅ GCC instalado com sucesso! Por favor, reinicie este prompt para atualizar o PATH do sistema.\n");
            exit(0);
        } else {
            printf("❌ Erro ao instalar via winget. Por favor, instale o MinGW manualmente ou use o MSYS2.\n");
            exit(1);
        }
    }
#else
    // 2. Verifica se o compilador cross-compile do MinGW está disponível no Linux
    ...
    if (!temMinGw) {
        ...
        // Atualiza a lista e instala o compilador de recursos e o gcc cross-compile de uma vez só
        int res = system("sudo apt update && sudo apt install -y mingw-w64 binutils-mingw-w64 wine wine64");
        
        if (res == 0) {
            printf("✅ Todos os compiladores e o Wine foram instalados com sucesso no seu Zorin OS! GG.\n");
        } else {
...
void compilarProjeto(const char* nome) {
    ...
    // Determina o nome real do executável/App
    const char* nomeExecutavel = (strcmp(nome, ".") == 0) ? "projeto" : nome;
    
    // Define os prefixos de caminhos de forma limpa
    ...
    if (res == 0) {
        ...
        // 🌟 INÍCIO DO ESPELHAMENTO DO SCRIPT DE PACKAGING LINUX NO WINDOWS 🌟
        ...
        if (fIcon) {
            ...
            sprintf(cmdWgetIcon, "powershell -Command \"Invoke-WebRequest -Uri 'https://upload.wikimedia.org/wikipedia/commons/8/83/Circle-icons-dev.svg' -OutFile '%s\\output\\linux\\%s.AppDir\\%s.svg' -Quiet\"", pathPrefixo, nomeExecutavel, nomeExecutavel);
            system(cmdWgetIcon);
        }
        ...
        if (!fWine) {
            ...
            char cmdWineDl[1024];
            sprintf(cmdWineDl, "powershell -Command \"Invoke-WebRequest -Uri 'https://github.com/Kron4ek/Wine-Builds/releases/download/10.0/wine-10.0-staging-amd64.tar.xz' -OutFile '%s\\tools\\wine-portable.tar.xz'\"", pathPrefixo);
            system(cmdWineDl);
        } else {
            fclose(fWine);
        }
        ...
        // Escrita em modo "wb" força quebras de linha em \n válidas para o Linux
        ...
        if (fAppRun) {
            ...
            // --- INJEÇÃO DO MAPA REGEDIT DO TEMA COMPLETO WINDOWS 11 FLAT ---
            ...
            fclose(fAppRun);
            system("powershell -Command \"Unblock-File -Path 'output\\linux\\*\\AppRun'\" 2>nul");
        }
        ...
        if (!fRuntime) {
            ...
            char cmdRtDl[1024];
            sprintf(cmdRtDl, "powershell -Command \"Invoke-WebRequest -Uri 'https://github.com/AppImage/AppImageKit/releases/download/continuous/runtime-x86_64' -OutFile '%s\\tools\\runtime-x86_64'\"", pathPrefixo);
            system(cmdRtDl);
        } else {
            fclose(fRuntime);
        }
...
#ifdef _WIN32
    printf("❌ Erro: O empacotamento distribuidor (.deb e .exe via InnoSetup) precisa do ambiente host Linux para rodar os scripts de ferramentas.\n");
#else
    printf("📦 [DEPLOY] Iniciando construção dos instaladores de distribuição pública...\n");
    char cmdDeb[512], cmdWin[512];
    sprintf(cmdDeb, "bash \"%s/tools/build_debian.sh\" \"%s\"", nome, nome);
    sprintf(cmdWin, "bash \"%s/tools/build_windows_setup.sh\" \"%s\"", nome, nome);
