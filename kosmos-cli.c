...
void mostrarAjuda() {
    ...
    printf("  run [nome_projeto]      ⚡ Compilação RÁPIDA e executa direto (Sem gerar AppImage)\n");
    printf("  debug [nome_projeto]    🪲 Compila em modo CONSOLE de debug para rastrear crashes\n");
    printf("  build [nome_projeto]    📦 Compilação COMPLETA e gera pacote estável AppImage\n");
    printf("  deploy [nome_projeto]   Gera os pacotes de instalação final .deb e .exe\n");
...
void criarAmbienteVscode(const char* nomeProjeto) {
    ...
    if (t) {
#ifdef _WIN32
        fprintf(t, "{\n  \"version\": \"2.0.0\",\n  \"tasks\": [\n");
        fprintf(t, "    {\n      \"label\": \"KosmosUI: Fast Run (Windows)\",\n      \"type\": \"shell\",\n      \"command\": \"kosmos run\",\n");
        ...
        fprintf(t, "    {\n      \"label\": \"KosmosUI: Debug Mode (Windows)\",\n      \"type\": \"shell\",\n      \"command\": \"kosmos debug\",\n");
        fprintf(t, "      \"problemMatcher\": [\"$gcc\"],\n");
        fprintf(t, "      \"presentation\": { \"echo\": true, \"reveal\": \"always\", \"focus\": true, \"panel\": \"shared\", \"clear\": true }\n    }\n  ]\n}");
#else
        fprintf(t, "{\n  \"version\": \"2.0.0\",\n  \"tasks\": [\n");
        fprintf(t, "    {\n      \"label\": \"KosmosUI: Fast Run (Linux)\",\n      \"type\": \"shell\",\n      \"command\": \"kosmos run\",\n");
        fprintf(t, "      \"group\": { \"kind\": \"build\", \"isDefault\": true },\n");
        fprintf(t, "      \"problemMatcher\": [\"$gcc\"],\n");
        fprintf(t, "      \"presentation\": { \"echo\": true, \"reveal\": \"always\", \"focus\": true, \"panel\": \"shared\", \"clear\": true }\n    },\n");
        fprintf(t, "    {\n      \"label\": \"KosmosUI: Debug Mode (Linux)\",\n      \"type\": \"shell\",\n      \"command\": \"kosmos debug\",\n");
        fprintf(t, "      \"problemMatcher\": [\"$gcc\"],\n");
        fprintf(t, "      \"presentation\": { \"echo\": true, \"reveal\": \"always\", \"focus\": true, \"panel\": \"shared\", \"clear\": true }\n    }\n  ]\n}");
#endif
        fclose(t);
...
void criarProjeto(const char* nome) {
    ...
    printf("👉 Execute: kosmos run \"%s\" para compilar e rodar instantaneamente de forma leve.\n", nome);
}

// 🌟 FUNÇÃO DE COMPILAÇÃO REUTILIZÁVEL E UNIFICADA (Suporta compilar Completo, Rápido e Debug)
void executarPipelineBuild(const char* nome, int gerarAppImage, int modoDebug) {
    ...
    char cmdRes[1024];
    // Adicionado flag --preprocessor=gcc para blindar o windres
    sprintf(cmdRes, "windres --preprocessor=gcc -I \"%s\" -i \"%s\" -o \"%s\" -O coff -F pe-x86-64", pastaResource, rcTemporario, arquivoObjeto);
    ...
    if (res == 0) {
        ...
        if (gerarAppImage) {
            // Bloco completo de geração do AppImage quando chamado via `kosmos build`
            ...
            printf("[4/7] Criando AppRun Dinâmico...\n");
            ...
            printf("[6/7] Verificando Runtime do AppImage...\n");
            ...
            if (rSquash != 0) { printf("ERRO: mksquashfs não encontrado.\n"); exit(1); }
            ...
            printf(" ✅ SUCESSO! AppImage criado em: %s\\output\\linux\\%s-x86_64.AppImage\n", pathPrefixo, nomeExecutavel);
        }

        // 🚀 AUTO-RUN DO WINDOWS HOST
        printf("\n🚀 [AUTO-RUN] Executando binário nativo local...\n");
        char cmdRunWin[1024];
        if (strcmp(nome, ".") == 0) {
    ...
    sprintf(cmdGcc, "/usr/bin/x86_64-w64-mingw32-gcc -g -Wall -D_WIN32_WINNT=0x0A00 -DWINVER=0x0A00 -DUNICODE -D_UNICODE -Wl,--subsystem,%s -municode -I \"%s\" -I \"%s\" \"%s\" \"%s\" \"%s\" -o \"%s\" -lcomctl32 -lshcore -lgdi32 -luser32 -lshlwapi -lgdiplus -static-libgcc -static-libstdc++ -lcomdlg32", 
            flagSubsystem, pastaResource, pastaKosmos, arquivoMain, arquivoCore, arquivoObjeto, arquivoSaida);
    ...
    if (gerarAppImage) {
        ...
        r4 = system(cmdPackage);
    } else {
        printf("⚡ [3/3] Pulando empacotamento pesado SquashFS (Modo Fast Run ativo!).\n");
    }

    if (r1 == 0 && r2 == 0 && r4 == 0) {
        printf("✅ [Sucesso] Sequência de compilação executada com êxito!\n");
        printf("🚀 [AUTO-RUN] Inicializando aplicação instantaneamente...\n");
        
        char cmdRunLinux[1024];
        if (gerarAppImage) {
            // Executa o AppImage completo estável
            if (strcmp(nome, ".") == 0) {
            ...
            // 🌟 FAST RUN NO LINUX: Roda o .exe direto pelo Wine nativo sem demorar compilando a árvore SquashFS!
            const char* envDebug = modoDebug ? "WINEDEBUG=+relay,+msg" : "WINEDEBUG=-all";
            if (strcmp(nome, ".") == 0) {
                sprintf(cmdRunLinux, "%s wine \"%s\"", envDebug, arquivoSaida);
            } else {
                sprintf(cmdRunLinux, "cd \"%s\" && %s wine \"output/%s.exe\"", pathPrefixo, envDebug, nomeExecutavel);
            }
        }
        system(cmdRunLinux);
    } else {
...
// Mapeamento dos comandos legados mantendo compatibilidade retroativa
void compilarProjeto(const char* nome) {
    executarPipelineBuild(nome, 1, 0); // Modo Build Tradicional (Gera AppImage, Oculta Console)
}
...
int main(int argc, char* argv[]) {
    ...
    if (strcmp(argv[1], "create") == 0) {
        ...
        // 🌟 NOVO: Compilação ultraleve e direta sem AppImage
        const char* projetoAlvo = (argc < 3) ? "." : argv[2];
        executarPipelineBuild(projetoAlvo, 0, 0);
    }
    else if (strcmp(argv[1], "debug") == 0) {
        // 🌟 NOVO: Ativa o console de log por trás (-Wl,--subsystem,console) e logs extras do Wine
        const char* projetoAlvo = (argc < 3) ? "." : argv[2];
        executarPipelineBuild(projetoAlvo, 0, 1);
    }
